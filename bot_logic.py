...
class CryptoBot:
    ...
    def create_features(self, df):
        ...
        if adx is not None:
            ...
            df["adx"] = 0  # Set default value to 0
    ...
    def train_model(self, df):
        features = ["ema_9", "ema_21", "rsi_7", "macd_hist", "adx", "atr", "vwap"]  # Use more expressive features
        X = df[features]
        y = df["target"]
        ...
        model = XGBClassifier(n_estimators=300, learning_rate=0.01)  # Replace RandomForest with XGBoost
        model.fit(X_train, y_train)
        y_pred = model.predict(X_test)
        score = f1_score(y_test, y_pred)  # Evaluate model using F1-Score
        logger.info(f"Model F1-Score: {score:.2f}")
        return model, score
    ...
    def analyze_symbol(self, symbol, interval):
        ...
        # 4-hour timeframe data to determine the general trend
        klines_data_4h = self.fetch_klines(symbol, "4h")
        if not klines_data_4h or len(klines_data_4h) < 2:
            logger.warning(f"No klines data or not enough data for {symbol} on 4h interval")
            return None

        df_4h = pd.DataFrame(klines_data_4h, columns=["time", "open", "high", "low", "close", "volume", "close_time", "quote_asset_volume", "number_of_trades", "taker_buy_base_asset_volume", "taker_buy_quote_asset_volume", "ignore"])
        df_4h["close"] = pd.to_numeric(df_4h["close"], errors='coerce')
        df_4h["open"] = pd.to_numeric(df_4h["open"], errors='coerce')
        ...
        df_4h = self.create_features(df_4h)
        df_4h.bfill(inplace=True)  # Use bfill() instead of fillna(method='bfill')

        if len(df_4h) < 2:
            logger.warning(f"Not enough data after feature creation for {symbol} on 4h interval")
            return None
        ...
        if model is not None:
            model_prediction = model.predict(df_15m[["ema_9", "ema_21", "rsi_7", "macd_hist", "adx", "atr", "vwap"]].iloc[-1:])[0]
        else:
            model_prediction = 0
        ...
        if (
            ...
            (ema_9 > ema_21) and
            (ema_50_1h > ema_200_1h) and
            #(ema_50_4h > ema_200_4h) and
            (volume > 20) and
            #(close_price <= ema_21 * 1.005) and
            (close_price > r1) #and
            #(model_prediction == 1)
        ):
            signal = 1
